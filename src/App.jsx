...
// Determine Epic URL slugs up front
...
export default function App() {
  ...
  return (
    <div className="app-root">
      <header>
        <h1>Product: {prd.split("\n")[0] || "PRD-Aligned Backlog App"}</h1>
        <nav>
      ...
      <main>
        <Routes>
          <Route
            path="/"
            element={<Navigate to={epicSlugs.length > 0 ? `/epic/${epicSlugs[0].slug}` : "/notfound"} replace />}
          />
          {epicSlugs.map(({ slug, epic }) => (
            <Route key={slug} path={`/epic/${slug}`} element={<EpicPage epic={epic} prd={prd} />} />
          ))}
