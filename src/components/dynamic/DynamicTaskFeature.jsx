...
// Helper registry map for global state mock for this session
const globalMemory = {
  users: [
...
/**
 * Returns a React component that implements functionality
 * per dynamic analysis of the task name/description.
 */
export default function DynamicTaskFeature({ task, allTasks }) {
  ...
  if (
    ...
    // is "Remove user" present as a task under this story?
  ...
  if (
    ...
    // This is handled as part of UsersList as above!
    return null;
  }
...
function RegistrationForm({ submitTask }) {
  ...
  const handleSubmit = e => {
    ...
    // Mock API: add to globalMemory.users and success
    setTimeout(() => {
      const newUser = {
